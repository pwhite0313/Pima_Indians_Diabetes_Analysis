...
#Briefly analyze the first 10 records in the dataset.
...
# The shape of the dataframe provides us with simple, yet useful information. We can confirm that there are indeed 9 columns, 8 of them features, or parameters, and the final being column indicating if the patient had diabetes at the time of the data collection.
# 
...
# All of the fields in the dataset are complete. There are no missing variables or null values.
...
# <br>For example: <br>
#     -The minimum values for BloodPressure, Glucose, SkinThickness, Insulin, BMI are 0. This is not possible and will need to be addressed for proper analysis and modeling.<br>
#     -The Max values should also be further analyzed to determine if they are either outliers, or anomalies within the dataset.

# # We will review and replace anomalies, inconsistencies, and missing data in order to move forward with proper data analysis
...
# As we do not have the necessary information within this dataset to calculate the BMI of each observation, we will replace the missing 'Insulin' values with that of the median of all observations of this feature.
...
# ## The Insulin variable has changed the most out of all of the features. This parameter has the highest standard deviation so we will accept the new values. The minimal changes in the remaining summary statistics show that our choice to replace all '0' values with that of the median for each feature has proven to be an effective solution.

# # With all anomalies and inconsistencies in the dataset resolved, we will move forward with the data analysis.
...
# Using the above distribution plot, we can see that the Probability Density of the variable 'BloodPressure' is mostly distributed between the values 40 and 100 (mm Hg).
...
# 37 women observed have their 'BloodPressure' equal to the median of 'BloodPressure' AND their 'BMI' less than the median of 'BMI'. We must be cautious when analyzing data whose features were not observed and have been manually altered.<br>
# In this instance, the 'BloodPressure' median was used to fill in all values of 0, as well as BMI. As we are not comparing 2 medians (which would return all observations meeting this condition, as well as the observations where both of these features were changed to the median), we can use this information with caution
...
#Use Matplotlib to create a boxplot for the 'Age' variable.
...
#We will use Matplotlib to histograms for variable Age to understand the number of women in different Age groups given that they have diabetes or not.
...
# The top histogram is a visualization of the distribution of Age for Women who have Diabetes. We see a negative correlation between age and having diabetes. We can further visualize that a higher frequency of those surveyed who have diabetes are more likely to be younger in age.
# 
...
# 
# By comparing both histograms and paying particular attention to the frequencies of each chart, we see that the frequency of those surveyed, regardless of diabetic status, has a right skew in age along the x-axis. This distribution of age is further supported by the mean age variable of 33.240885 from a range of 21 to 81. Simply put: more women of younger age were surveyed.
...
# The above heatmap provides insight as to the correlation between each variable of the experiment, excluding 'Outcome'. Furthermore, the heatmap shows a majority of weak correlations between the majority of these chosen variables. The strongest positive correlation on the heatmap is between 'Age' and 'Pregnancies'. 'Age' also has weak positive correlation between 'Glucose' and 'BloodPressure'. 'Glucose' and 'Insulin' are the next variables that provide any noteworthy correlations between other variables. What we can gather from this heatmap is that the 'Glucose' has the highest correlation with 'Outcome', or having diabetes. Other variables correlate with one another, as previously noted; however we cannot extend these correlations to their relationship with the 'Outcome' variable.
